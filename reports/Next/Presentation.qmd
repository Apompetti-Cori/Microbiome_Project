...
count <- as.data.frame(colData(se)) %>% filter(genotype == "ApcMin/IL10KO") %>%
  dplyr::select(mouse_id, treatment, microbiome, suborgan, sampletype) %>%
  unique() %>%
  replace_na(list(treatment = "None")) %>%
  dplyr::count(microbiome, sampletype, suborgan)

count <- pivot_wider(count, names_from = c("microbiome", "sampletype", "suborgan"), values_from = "n", names_glue = "{microbiome}.{sampletype}.{suborgan}")
...
tab_1 |> gtsave(here::here("tab_1.png"), expand = 10)
```
...
p1 <- p_results$gf_by_suborgan$all + 
  labs(title = "GF:Proximal-Distal")
p2 <- p_results$consortium_by_suborgan$all +
  labs(title = "Consortium:Proximal-Distal")
p3 <- p_results$spf_by_suborgan$all + 
  labs(title="SPF:Proximal-Distal")
p4 <- p_results$eckp_by_suborgan$all +
  labs(title="ECKP:Proximal-Distal")

all <- wrap_elements(p1 | p2 | p3 | p4)

p1 <- p_results$gf_by_suborgan$`non-islands` + 
  labs(title = "GF:Proximal-Distal")
p2 <- p_results$consortium_by_suborgan$`non-islands` +
  labs(title = "Consortium:Proximal-Distal")
p3 <- p_results$spf_by_suborgan$`non-islands` + 
  labs(title="SPF:Proximal-Distal")
p4 <- p_results$eckp_by_suborgan$`non-islands` +
  labs(title="ECKP:Proximal-Distal")

noncgi <- wrap_elements(p1 | p2 | p3 | p4)

p1 <- p_results$gf_by_suborgan$islands + 
  labs(title = "GF:Proximal-Distal")
p2 <- p_results$consortium_by_suborgan$islands +
  labs(title = "Consortium:Proximal-Distal")
p3 <- p_results$spf_by_suborgan$islands + 
  labs(title="SPF:Proximal-Distal")
p4 <- p_results$eckp_by_suborgan$islands +
  labs(title="ECKP:Proximal-Distal")

cgi <- wrap_elements(p1 | p2 | p3 | p4)

p <- all / noncgi / cgi

p + 
  patchwork::plot_annotation(tag_levels = list(c("All Sites", "Non CGI", "CGI"))) &
...
### Volcano plot No Side
```{r fig.width=25, fig.height=15, dpi = 300}
p1 <- p_results$cons_gf$all + 
  labs(title = "Consortium-GF")
p2 <- p_results$spf_gf$all +
  labs(title = "SPF-GF")
p3 <- p_results$eckp_gf$all + 
  labs(title="ECKP-GF")

all <- wrap_elements(p1 | p2 | p3)

p1 <- p_results$cons_gf$`non-islands` + 
  labs(title = "Consortium-GF")
p2 <- p_results$spf_gf$`non-islands` +
  labs(title = "SPF-GF")
p3 <- p_results$eckp_gf$`non-islands` + 
  labs(title="ECKP-GF")

noncgi <- wrap_elements(p1 | p2 | p3)

p1 <- p_results$cons_gf$islands + 
  labs(title = "Consortium-GF")
p2 <- p_results$spf_gf$islands +
  labs(title = "SPF-GF")
p3 <- p_results$eckp_gf$islands + 
  labs(title="ECKP-GF")

cgi <- wrap_elements(p1 | p2 | p3)
...
```{r fig.width=30, fig.height=15, dpi = 300}
p1 <- p_results$gf_by_suborgan$all + 
  labs(title = "GF:PCOL-DCOL")
p2 <- p_results$consortium_by_suborgan$all +
  labs(title = "CONS:PCOL-DCOL")
p3 <- p_results$spf_by_suborgan$all + 
  labs(title="SPF:PCOL-DCOL")
p4 <- p_results$eckp_by_suborgan$all +
  labs(title="ECKP:PCOL-DCOL")
...
a <- as.data.frame(contrastResults(pj_normal, "eckp_gf")) %>%
  filter(sig & status == "hypo") %>%
  pull(chr_base)

# b <- as.data.frame(contrastResults(pj_normal, "consortium_by_suborgan")) %>%
#   filter(sig & status == "hyper" & is_island) %>%
#   pull(chr_base)
...
```{r}
overlap_results <- readRDS(here::here("results/Microbiome_Tumor_Proj/05/overlap_results.rds"))
```
...
### (GF: PCOL vs DCOL CONS: PCOL vs DCOL) &#x222A; (SPF: PCOL vs DCOL) 
```{r, fig.height=10, fig.width=20, dpi=300, out.width="75%"}
p1 <- overlap_results$contrast10a$plot +
  xlab("diffMeth A (GF/CONS: PCOL vs DCOL)") + ylab("diffMeth B (SPF: PCOL vs DCOL)") +
  theme(
    legend.position = "bottom",
    legend.title = element_text(size = 25, face = "bold"),
    axis.title.x = element_text(size = 22, face = "bold"),
...
p2 <- overlap_results$contrast10a$plot_facet

p3 <- overlap_results$contrast10a$ab_plot

p4 <- overlap_results$contrast10a$plot_facet +
  xlab("diffMeth A (GF/CONS: PCOL vs DCOL)") + 
  ylab("diffMeth B (SPF: PCOL vs DCOL)")
...
### (GF: PCOL vs DCOL CONS: PCOL vs DCOL) &#x222A; (ECKP: PCOL vs DCOL) 
```{r, fig.height=10, fig.width=20, dpi=300, out.width="75%"}
p1 <- overlap_results$contrast1b$plot +
  xlab("diffMeth A (GF/CONS: PCOL vs DCOL)") + ylab("diffMeth B (ECKP: PCOL vs DCOL)") +
  theme(
    legend.position = "bottom",
    legend.title = element_text(size = 25, face = "bold"),
    axis.title.x = element_text(size = 22, face = "bold"),
...
p2 <- overlap_results$contrast1b$plot_facet

p3 <- overlap_results$contrast1b$ab_plot

p4 <- overlap_results$contrast1b$plot_facet +
  xlab("diffMeth A (GF/CONS: PCOL vs DCOL)") + 
...
```{r,fig.width=20, fig.height=10}
dm_results <- readRDS(here::here("results/Microbiome_Tumor_Proj/03/dm_results.rds"))
a <- dm_results$gf_by_suborgan %>% filter(sig) %>% pull(chr_base)
b <- dm_results$consortium_by_suborgan %>% filter(sig) %>% pull(chr_base)

sites <- BiocGenerics::union(a,b)

genes <- data.frame(unlist(granges(pj_normal[sites,]))) %>%
  filter(id == "promoter") %>%
  pull("gene_id") %>%
  gsub("\\.[0-9]+", "", .) 
...
a <- rownames(subset(pj_normal, spf_gf))
b <- rownames(subset(pj_normal, eckp_gf))
...
### Sites unique to SPF:PCOL-DCOL
```{r}
testa <- as.data.frame(mcols(pj_normal)) %>% filter((gf_by_suborgan | consortium_by_suborgan)) %>% pull(chr_base)
testb <- as.data.frame(mcols(pj_normal)) %>% filter(spf_by_suborgan) %>% pull(chr_base)

sites <- intersect(testb, testa)
...
### Site common between GF CONS AND SPF : PCOL-DCOL
```{r}
testa <- as.data.frame(mcols(pj_normal)) %>% filter((gf_by_suborgan | consortium_by_suborgan)) %>% pull(chr_base)
testb <- as.data.frame(mcols(pj_normal)) %>% filter(spf_by_suborgan) %>% pull(chr_base)
...
testc <- as.data.frame(mcols(pj_normal)) %>% 
...
### Site unqiue to ECKP : PCOL-DCOL
```{r}
testa <- as.data.frame(mcols(pj_normal)) %>% filter((gf_by_suborgan | consortium_by_suborgan)) %>% pull(chr_base)
testb <- as.data.frame(mcols(pj_normal)) %>% filter(eckp_by_suborgan) %>% pull(chr_base)

testc <- setdiff(testb,testa)
...
### Site common between GF CONS ECKP : PCOL-DCOL
```{r}
testa <- as.data.frame(mcols(pj_normal)) %>% filter((gf_by_suborgan | consortium_by_suborgan)) %>% pull(chr_base)
testb <- as.data.frame(mcols(pj_normal)) %>% filter(eckp_by_suborgan) %>% pull(chr_base)

testc <- intersect(testb,testa)
...
pca_res <- readRDS(here::here("results/Microbiome_Tumor_Proj/04/pca_results.rds"))
p_results <- sapply(
  list("noSide" = pca_res$pca_noside, "noSideCGI" = pca_res$pca_noside_cgi, "noSidenonCGI" = pca_res$pca_noside_noncgi, 
       "side" = pca_res$pca_side, "sideCGI" = pca_res$pca_side_cgi, "sidenonCGI" = pca_res$pca_side_noncgi),
  FUN = function(pca) {
    p1 <- ggplot(data = pca$rotated, aes(PC1, PC2, color = factor(pca$metadata$microbiome,
      levels = c(
        "gf", "consortium",
...
### Overlap ECKP-GF vs SPF-GF
```{r, fig.height=10, fig.width=20, dpi=300, out.width="75%"}
overlap_results <- readRDS(here::here("results/Microbiome_Tumor_Proj/05/overlap_results_filter_side.rds"))

p1 <- overlap_results$contrast1a$plot +
...
p4 <- overlap_results$contrast1a$plot_facet +
  xlab("diffMeth A (ECKP vs GF)") + ylab("diffMeth B (SPF vs GF)") +
  theme(
    legend.position = "bottom",
    legend.title = element_text(size = 25, face = "bold"),
...
fit <- lm(avg_logRPKM.PCOL ~ pcol_mean, data = data)
p1 <- ggplot(data = data, 
...
fit <- lm(avg_logRPKM.DCOL ~ dcol_mean, data = data)
p2 <- ggplot(data = data, 
...
```{r, fig.height=8, fig.width=20, dpi = 300}
fit <- lm(avg_logRPKM.PCOL ~ as.numeric(pcol_t2), data = data)
...
p3 <- ggplot(data = data, 
  ...
  scale_y_continuous(limits = c(-12,12)) +
  scale_x_discrete(limits = c(FALSE, TRUE), 
                   labels = c("Unmethylated (0-0.1)",
                              "Methylated (0.1-1.0)"))
  
fit <- lm(avg_logRPKM.DCOL ~ as.numeric(dcol_t2), data = data)
...
p7 <- ggplot(data = data, aes(x = factor(pcol_methylated, c(TRUE, FALSE)),
  ...
  scale_x_discrete(limits = c(FALSE, TRUE), labels = c("Unmethylated (0-0.1)",
                                                       "Methylated (0.1-1.0)")) + 
  scale_fill_discrete(labels = c("Expressed", "Unexpressed")) +
  xlab("Gene Methylation Status Proximal Colon") + 
  ylab("prop") +
...
## PCA
```{r}
se <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/GSE186458"), "isee")

meth <- as.matrix(assay(se, "Meth"))
cov <- as.matrix(assay(se, "Cov"))

pca <- PCAtools::pca(meth[!rowAnyNAs(meth, useNames = FALSE),], metadata = colData(se))

p <- ggplot(data = pca$rotated, aes(PC1, PC2, color = pca$metadata$tissue)) +
  geom_point(size = 10, alpha = .9) +
  scale_color_manual(
    name = "Tissue Type",
    values = c("forestgreen", "khaki"),
...
ggplot(cov,aes(value))+geom_histogram(bins=30)+scale_y_log10()+facet_grid(.~Var2)
```
...
ggplot(meth,aes(value))+geom_histogram(bins=30)+facet_grid(.~Var2)
```

## Volcano Plot
```{r, fig.width=16, fig.height=6}
se <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/GSE186458"), prefix = "isee")

p <- coriell::plot_volcano(
  contrastResults(se, "by_suborgan"),
  x = "diff", 
  y = "adj.P.Val", 
  lfc = .1, 
  fdr = .1
...
## Boxplot
```{r, fig.height=8, fig.width=16}
meth <- as.matrix(assay(subset(se, sig), "Meth"))  %>% 
...
meth <- left_join(
  ...
  mutate(tissue = factor(tissue, levels = c("PCOL", "DCOL")))

meth <- left_join(
  meth, 
  data.frame(contrastResults(subset(se, sig), "by_suborgan")),
  by = "chr_base"
)

p1 <- ggplot(
  meth %>% filter(status == "hypo"), 
  mapping = aes(
    x = tissue,
    y = value,
    fill = tissue)
) + 
  geom_violin(width = 1, linewidth = 1.25, alpha = 1) +
  geom_boxplot(width = 0.1, linewidth = 1.25,  show.legend = FALSE, fill = "white") + 
  scale_fill_manual(
    values = c("khaki", "forestgreen"),
    name = "Tissue Type",
    labels = c("PCOL" = "Proximal", "DCOL" = "Distal")
  ) +
...
## GO enrichment methylation
```{r, fig.width=15, fig.height=6}
genes <- subset(unlist(granges(subset(se, sig))), id == "promoter")
geneList <- gsub("\\.[0-9]+", "", genes$gene_id)
...
listDatasets(mouse)
searchDatasets(mouse, "musculus")
mouse <- useDataset("mmusculus_gene_ensembl", mouse)

human <- useMart(biomart = "ENSEMBL_MART_ENSEMBL",
                   host = "https://apr2020.archive.ensembl.org")
listDatasets(human)
searchDatasets(human, "sapien")
human <- useDataset("hsapiens_gene_ensembl", human)

se <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/GSE186458"), "isee")
se2 <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/rrbs/"), "isee")
se2 <- subset(se2, spf_by_suborgan)

gene_ids <- subset(unlist(granges(se)), id == "promoter")$gene_id

geneList <- data.frame(
  gene_id = unique(na.omit(gsub("\\.[0-9]*","",gene_ids))),
  stable_id = unique(na.omit(gene_ids))
)

gene_ids <- subset(unlist(granges(se2)), id == "promoter")$gene_id

geneList2 <- data.frame(
  mouse_gene_id = gsub("\\.[0-9]*","",unique(na.omit(gene_ids))),
  mouse_stable_id = unique(na.omit(gene_ids))
)

conv <- getLDS(attributes = c("ensembl_gene_id"), 
                   filters = "ensembl_gene_id",
                   values = geneList2$mouse_gene_id,
                   mart = mouse,
                   attributesL = c("ensembl_gene_id"),
                   martL = human,
                   uniqueRows=T)
colnames(conv) <- c("mouse_gene_id", "gene_id")

x <- left_join(geneList2, conv, by = "mouse_gene_id")
x <- left_join(x, geneList, by = "gene_id") %>% na.omit()

#Get methylation of mouse promoters
sites <- unique(names(subset(unlist(granges(se2)), gene_id %in% x$mouse_stable_id 
                             & id == "promoter")))
meth <- data.frame(
  chr_base = rownames(se2[sites,]),
  assay(se2[sites,se2$microbiome == "spf"], "Meth"),
  contrastResults(se2[sites,], "spf_by_suborgan")[,c("group1", "group2","diff")]
)

genes <- data.frame(mcols(unlist(granges(se2[sites,])))) %>%
  filter(id == "promoter") %>%
  distinct(chr_base, gene_id, .keep_all = TRUE)

mouse_meth <- left_join(genes, meth, by = "chr_base") %>%
  dplyr::rename(mouse_stable_id = gene_id)

mouse_meth <- left_join(mouse_meth, x, by = "mouse_stable_id") %>%
  dplyr::group_by(mouse_gene_id) %>%
  dplyr::mutate(avg_meth_group1_mm = mean(group1),
                avg_meth_group2_mm = mean(group2)) %>%
  na.omit() %>%
  dplyr::select(-mouse_stable_id, -stable_id)

#Get methylation of human promoters
sites <- unique(names(subset(unlist(granges(se)), gene_id %in% x$stable_id 
                             & id == "promoter")))
meth <- data.frame(
  chr_base = rownames(se[sites,]),
  assay(se[sites,], "Meth"),
  contrastResults(se[sites,], "by_suborgan")[,c("group1", "group2","diff")]
)

genes <- data.frame(mcols(unlist(granges(se[sites,])))) %>%
  filter(id == "promoter" & type == "gene") %>%
  distinct(chr_base, gene_id, .keep_all = TRUE)

human_meth <- left_join(meth, genes, by = "chr_base") %>%
  dplyr::rename(stable_id = gene_id)

human_meth <- left_join(human_meth, x, by = "stable_id") %>%
  dplyr::group_by(gene_id) %>%
  dplyr::mutate(avg_meth_group1_hs = mean(group1),
                avg_meth_group2_hs = mean(group2)) %>%
  na.omit() %>%
  dplyr::select(-mouse_stable_id, -stable_id)

data <- left_join(mouse_meth, human_meth, by = c("mouse_gene_id", "gene_id"), 
                  suffix = c(".mm", ".hs")) %>%
  dplyr::select(mouse_gene_id, gene_id, 
         avg_meth_group1_mm, avg_meth_group2_mm, 
         avg_meth_group1_hs, avg_meth_group2_hs,
         diff.mm, diff.hs) %>%
  distinct(mouse_gene_id, gene_id, .keep_all = TRUE)

data_longer <- data %>%
  pivot_longer(cols = c(avg_meth_group1_mm, avg_meth_group2_mm, 
                        avg_meth_group1_hs, avg_meth_group2_hs))
...
ggplot(
  data,
  aes(
    x = avg_meth_group1_mm,
    y = avg_meth_group1_hs
  )
) + 
  geom_point() +
  xlab("Avg Meth PCOL Mm") +
  ylab("Avg Meth PCOL Hs") +
  theme_classic()

ggplot(
  data,
  aes(
    x = avg_meth_group2_mm,
    y = avg_meth_group2_hs
  )
) + 
  geom_point() +
  xlab("Avg Meth DCOL Mm") +
  ylab("Avg Meth DCOL Hs") +
  theme_classic() +
...
se <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/GSE186458/"), "isee")
se2 <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/rrbs/"), "isee")
chainfile <- import.chain(here::here("data/chainfiles/mm10ToHg38.over.chain"))
results <- as.data.frame(liftOver(granges(se2), chainfile)) %>% 
  mutate(chr_base2 = paste0(seqnames,"_",start)) %>%
  dplyr::select(chr_base, chr_base2) %>%
  distinct(chr_base, chr_base2)

test <- results %>% filter(chr_base2 %in% rownames(se)) %>% distinct(chr_base2, .keep_all = TRUE)


test2 <- data.frame(contrastResults(se[test$chr_base2,], "by_suborgan")) %>% 
  dplyr::rename(chr_base2 = chr_base) %>% 
  left_join(., test, by = "chr_base2")
test3 <- data.frame(contrastResults(se2[test$chr_base,], "spf_by_suborgan")) %>% 
  left_join(., test2, by = "chr_base", suffix = c(".mouse", ".human"))

test4 <- test3 %>% 
  filter(sig)

plot(test3$diff.mouse, test3$diff.human)

mosaicplot(table(test3$status.mouse, test3$status.human))

plot(test3$group1.mouse, test3$group1.human)

plot(test3$group2.mouse, test3$group2.human)

plot(test3$group1.mouse, test3$group2.mouse)

plot(test3$group1.human, test3$group2.human)

#
plot(test4$diff.mouse, test4$diff.human)

plot(test4$group1.mouse, test4$group1.human)

plot(test4$group2.mouse, test4$group2.human)

mosaicplot(table(test4$status.mouse, test4$status.human))
```

## Age Correlation
```{r}
se <- HDF5Array::loadHDF5SummarizedExperiment(here::here("data/se/rrbs"), prefix = "isee")
age_cor <- readRDS(here::here("results/Microbiome_Tumor_Proj/08/age_cor.rds"))
data <- data.frame(contrastResults(se, "eckp_by_suborgan")[names(age_cor),])
data$age_cor <- age_cor

mean(abs(data[data$sig, "age_cor"]), na.rm = TRUE)
```