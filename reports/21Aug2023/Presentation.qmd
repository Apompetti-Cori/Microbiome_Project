...
# No Filter {visibility="hidden"}

```{r, include=FALSE, cache=TRUE, cache.lazy=FALSE}
#Load SE
...
cov <- getCoverage(se, type = "Cov")
rownames(cov) <- mcols(se)$chr_base
```

## Coverage of `r prettyNum(nrow(se), big.mark = ",", scientific = FALSE)` loci in spleen samples {auto-animate=true style="font-size: 20px;"}
...
#beachmat colBlock doesnt actually apply to each column individually so I need to specify a grid
grid <- colAutoGrid(cov, ncol = 1)
freq_bins <- blockApply(cov, FUN = fun, grid = grid, BPPARAM = BiocParallel::MulticoreParam(workers = 12))
...
## Methylation of `r prettyNum(nrow(se), big.mark = ",", scientific = FALSE)` loci in spleen samples {auto-animate=true style="font-size: 20px;"}
...
#beachmat colBlock doesnt actually apply to each column individually so I need to specify a grid
grid <- colAutoGrid(meth, ncol = 1)
freq_bins <- blockApply(meth, FUN = fun, grid = grid, BPPARAM = BiocParallel::MulticoreParam(workers = 12))
...
# Filter: Max coverage Inf, Min coverage 30, Percent coverage 75% {visibility="hidden"}
...
#### Max coverage Inf; Min coverage: 30; Percent coverage: 75%
...
# Filter: Max coverage 500, Min coverage 30, Percent coverage 75% {visibility="hidden"}
...
#### Max coverage 500; Min coverage: 30; Percent coverage: 75%
...
::: r-stack
::: fragment
```{r, fig.width=10,fig.height=10, out.height="50%"}
