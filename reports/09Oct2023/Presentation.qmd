...
for(name in names(results3)){
    df <- results3[[name]]
    test <- df[df$sig,"chr_base"]
    results3[[name]] <- contrasts3$chr_base %in% test
}
```
...
pca_meth <- meth[rownames(meth) %noin% sites,]
...
```{r geneont, include=FALSE, cache=TRUE}
go_results <- sapply(
  names(results),
  FUN = function(x){
    df <- results[[x]]
    ...
    if(nrow(head(go_results_hyper)) == 0){
    ...
    if(nrow(head(go_results_hypo)) == 0){
...
::: panel-tabset
### Volcano plot {style="font-size: 2px;"}
...
## PCA with sidedness removed {style="font-size: 16px;"}
```{r, fig.height=6, fig.width=15, dpi=300, out.width="50%"}
pca <- pca_noside
...
## Differential Methylation using `r prettyNum(nrow(pj_normal), big.mark = ",", scientific = FALSE)` sites {style="font-size: 16px;"}
#### Comparing microbiome vs germ-free (sidedness sites removed) {style="font-size: 16px;"}
```{r, include=FALSE}
p_results <- sapply(
  names(results)[12:14],
  FUN = function(x){
...
## Differential Methylation using `r prettyNum(nrow(pj_normal), big.mark = ",", scientific = FALSE)` sites {style="font-size: 16px;"}
#### Comparing microbiome separating sidedness (sidedness sites removed) {style="font-size: 16px;"}
```{r, include=FALSE}
p_results <- sapply(
  names(results[6:11]),
  FUN = function(x){
...
#### Comparing microbiome vs germ-free (sidedness retained) {style="font-size: 16px;", visibility="hidden"}
```{r, include=FALSE}
...
contrast.matrix <- limma::makeContrasts(
  contrast1 = eckp_gf - (by_suborgan + eckp_by_suborgan),
  contrast2 = spf_gf - (by_suborgan + eckp_by_suborgan),
  contrast3 = consortium_gf - (by_suborgan + consortium_by_suborgan),
  levels=design
)
```
...
overlap_results <- sapply(
  ...
  FUN = function(x){
    ...
    #calculate Avgsig of group1 of the contrast. (Only true if all elements in group1 are true)
    df1$Avgsig <- rowMeans(
    ...
    df3 <- inner_join(df1, df2, by = "chr_base") %>% 
      ...
      mutate(
        color = case_when(
          sign(Avgdiff.A) != sign(Avgdiff.B) ~ "purple",
          (sign(Avgdiff.A) > 0) & (sign(Avgdiff.B) > 0) ~ "red",
          (sign(Avgdiff.A) < 0) & (sign(Avgdiff.B) < 0) ~ "blue"
        )
      )
    ...
    p <- df3 %>%
      ...
      geom_point(data = df3[df3$color != "gray", ], alpha = .4, mapping = aes(color = color)) +
      ...
      labs(
        ...
        subtitle = bquote(bold("Overlap size:") ~ .(size) ~ bold("Pearson's coef:") ~ .(pearson))
        ) +
      theme_classic()
    
    return(list("df" = df3, "plot" = p))
  },
  simplify = FALSE,
  USE.NAMES = TRUE
)
...
::: panel-tabset
### ECKP-GF vs PCOL-DCOL {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast1$plot +
  xlab("diffMeth (ECKP vs GF)") + ylab("diffMeth (PCOL vs DCOL)")
```

### SPF-GF vs PCOL-DCOL {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast2$plot +
    xlab("diffMeth (SPF vs GF)") + ylab("diffMeth (PCOL vs DCOL)")
```

### CONS-GF vs PCOL-DCOL {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast3$plot + 
    xlab("diffMeth (CONS vs GF)") + ylab("diffMeth (PCOL vs DCOL)")
```
:::

## Overlap of Differential Methylation {style="font-size: 16px;"}
#### Comparing microbiome vs germ-free (sidedness removed) {style="font-size: 16px;", visibility="hidden"}
```{r, include=FALSE}
# Make constrast matrix
results <- c(results1, results2)
...
contrast.matrix <- limma::makeContrasts(
  contrast1 = eckp_gf - spf_gf, 
  contrast2 = eckp_gf - consortium_gf,
  contrast3 = spf_gf - consortium_gf,
  contrast4 = (gf_by_suborgan + consortium_by_suborgan + spf_by_suborgan + eckp_by_suborgan) - by_suborgan,
  levels=design
)
```
...
::: panel-tabset
### ECKP-GF vs SPF-GF {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast1$plot +
  xlab("diffMeth (ECKP vs GF)") + ylab("diffMeth (SPF vs GF)")
```

### ECKP-GF vs CONS-GF {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast2$plot +
    xlab("diffMeth (ECKP vs GF)") + ylab("diffMeth (Consortium vs GF)")
```

### SPF-GF vs CONS-GF {style="font-size: 2px;"}
```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast3$plot + 
    xlab("diffMeth (SPF vs GF)") + ylab("diffMeth (Consortium vs GF)")
```
:::

## Overlap of Differential Methylation {style="font-size: 16px;"}
#### Comparing PCOL vs DCOL (sidedness removed) {style="font-size: 16px;", visibility="hidden"}

```{r, fig.height=7, fig.width=8, dpi=300, out.width="50%"}
overlap_results$contrast4$plot +
  xlab("diffMeth (by_suborgan: gf+cons+spf+eckp)") + ylab("diffMeth (by_suborgan)")
```

## GO enrichment analysis {style="font-size: 16px;", visibility="hidden"} 
#### Comparing PCOL vs DCOL {style="font-size: 16px;", visibility="hidden"}
```{r, include=FALSE}
p_results <- sapply(
  names(go_results),
  FUN = function(x){
